...
class TopKTopPSampler(nn.Module):
    ...
    def forward_native(
        ...
        return random_sample_optimized(probs, generators), logits_to_return
    ...
    def forward_cuda(
        ...
        # We prefer `random_sample_optimized` over `flashinfer_sample` when sorting is
        # not needed. This is because `random_sample_optimized` does not require
...
def random_sample_optimized(
    ...
    """Optimized random sampling from the probabilities.
    
    This implementation batches the exponential sampling for requests with
    the same generator state, significantly reducing overhead when multiple
    requests share generators or when processing many requests.
    
    The optimization addresses the TODO comment about slow sequential processing
    by grouping and batch-processing requests with identical generator states.
    """
    ...
    if not generators:
        ...
        q.exponential_()
    elif len(generators) == batch_size:
        # All requests have custom generators
        # Group by unique generator state for batching
        state_to_info = {}
        
        for idx, gen in generators.items():
            # Get generator state as hashable key
            state = gen.get_state()
            state_key = hash(state.tobytes()) if isinstance(state, torch.Tensor) else hash(str(state))
            
            if state_key not in state_to_info:
                state_to_info[state_key] = ([], gen)
            state_to_info[state_key][0].append(idx)
        
        # Process each group in batch
        for indices, gen in state_to_info.values():
            if len(indices) == 1:
                # Single request - process directly
                q[indices[0]].exponential_(generator=gen)
            else:
                # Multiple requests with same generator - batch process
                indices_tensor = torch.tensor(indices, device=probs.device, dtype=torch.long)
                batch_q = torch.empty((len(indices), probs.shape[1]), 
        ...
        # First, generate default exponential for all
        q.exponential_()
        
        if generators:
            # Group generators by state for efficient batch processing
            state_to_info = {}
            ...
            # Batch process updates for custom generators
            for indices, gen in state_to_info.values():
                if len(indices) == 1:
                    q[indices[0]].exponential_(generator=gen)
                else:
...
def random_sample(
    ...
    if generators:
        ...
        # OPTIMIZATION: This has been addressed in random_sample_optimized
